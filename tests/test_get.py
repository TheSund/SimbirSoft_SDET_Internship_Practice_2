...
def test_get_entity(api_client: ApiClient, created_entity: tuple[int, dict]) -> None:
    ...
    with allure.step('Проверка успешного получения сущности'):
        assert response.status_code == 200, f'Ожидался статус 200, но получен {response.status_code}'

    with allure.step('Проверка соответствия сущности схеме'):
        try:
            parsed = Entity(**response.json())
        except Exception as e:
            raise AssertionError(f'Структура ответа не соответствует данным: {e}')

    with allure.step('Проверка соответствия данных сущности ожидаемым'):
